...
public class AsyncDrawable<T extends View> extends Drawable {
    ...
    private final WeakReference<WBitmap.BitmapLoadTask<T>> bitmapLoadTaskReference;

    private final Drawable baseDrawable;

    public AsyncDrawable(Drawable drawable, WBitmap.BitmapLoadTask<T> bitmapWorkerTask) {
        if (bitmapWorkerTask == null) {
            throw new IllegalArgumentException("bitmapWorkerTask may not be null");
        }
        baseDrawable = drawable;
        bitmapLoadTaskReference = new WeakReference<WBitmap.BitmapLoadTask<T>>(bitmapWorkerTask);
    }

    public WBitmap.BitmapLoadTask<T> getBitmapWorkerTask() {
