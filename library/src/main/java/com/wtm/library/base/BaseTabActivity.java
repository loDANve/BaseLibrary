...
public abstract class BaseTabActivity extends BaseActivity {
    ...
    protected void changeTab(int position){
        ...
        onTabChanged(currentTabPosition,position);
    ...
    protected  void initTab(){
        if (!fragmentList.get(0).isAdded()){
            getSupportFragmentManager().beginTransaction()
                    .add(container_fragment, fragmentList.get(0)).show(fragmentList.get(0)).commit();
        }
       /* if (!fragmentList.get(1).isAdded()){
            getSupportFragmentManager().beginTransaction()
                    .add(container_fragment, fragmentList.get(1)).hide(fragmentList.get(1)).commit();
        }*/
       /* getSupportFragmentManager().beginTransaction()
