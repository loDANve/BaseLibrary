 ...
 */
public class Staff_PriceAdjustmentListViewAdapter extends BaseAdapter {

    private Context context;

    private ArrayList<Goods> staffOrderDetailGoodsList;
    private WBitmap wBitmap;

    public Staff_PriceAdjustmentListViewAdapter(Context context, ArrayList<Goods> staffOrderDetailGoodsList) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        final Goods goods = staffOrderDetailGoodsList.get(position);

        viewHolder.mTvItemGoodsName.setText(goods.getName());
        if (StringUtils.isEmpty(goods.getName())) {
            viewHolder.mTvItemGoodsName.setText("");
        }
        viewHolder.mTvItemGoodsRemarks.setText(goods.getRemarks());
        if (StringUtils.isEmpty(goods.getRemarks())) {
            viewHolder.mTvItemGoodsRemarks.setText("");
        }
        viewHolder.mTvItemGoodsNumber.setText(goods.getNumber()+"");
        if (StringUtils.isEmpty(goods.getNumber()+"")) {
            viewHolder.mTvItemGoodsNumber.setText("");
        }
        if (!StringUtils.isEmpty(goods.getNumber()+"" ) && !StringUtils.isEmpty(goods.getMoney())) {
            viewHolder.mTvItemGoodsPrice.setText(""+goods.getNumber() * Integer.parseInt(goods.getMoney()));
        }

        /*if (!StringUtils.isEmpty(staffOrderDetail.getImgPath())) {
            WnbApplication.getInstance().getwBitmap().display(viewHolder.mImgItemGoodsImg, AppConfig.PATH_PREFIX+store.getImgPath());
        }*/
    ...
    static class ViewHolder {
        ...
        @InjectView(R.id.tv_staff_orderDetail_itemGoodsRemarks)
        TextView mTvItemGoodsRemarks;
